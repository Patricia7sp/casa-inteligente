...
class MiniAppKeyExtractor:
    ...
    def __init__(self):
        ...
        self.client_secret = os.getenv(
            "TUYA_CLIENT_SECRET", "021747b14008401f9f173dc693113aef"
        )
    ...
    def try_alternative_miniapp_methods(self):
        ...
        methods = [
            (
                "Direct device query",
                f"/v1.0/miniapps/{self.miniapp_id}/devices/{self.device_id}",
            ),
            ("App device list", f"/v1.0/apps/{self.miniapp_id}/devices"),
            ("MiniApp info", f"/v1.0/miniapps/{self.miniapp_id}"),
            (
                "Device via MiniApp",
                f"/v1.0/devices/{self.device_id}?miniapp_id={self.miniapp_id}",
            ),
        ]
