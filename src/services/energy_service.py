...
class EnergyAnalysisService:
    ...
    def calculate_daily_consumption(self, device_id: int, date: datetime) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if not readings:
                logger.warning(f"Nenhuma leitura encontrada para o dispositivo {device_id} na data {date}")
                return None
            ...
            total_energy_kwh = df["power_watts"].sum() / 1000 / 60  # Converter para kWh (assumindo leituras por minuto)
            ...
            runtime_hours = len(df[df["power_watts"] > 0]) / 60  # Assumindo leituras por minuto
    ...
    def detect_anomalies(self, device_id: int, current_consumption: float) -> Optional[Dict]:
        """
        ...
        try:
            ...
            avg_consumption = db.query(func.avg(EnergyReading.power_watts)).filter(
                and_(
                    EnergyReading.device_id == device_id,
            ...
            if current_consumption > avg_consumption * threshold_multiplier:
                ...
                logger.warning(f"Anomalia detectada no dispositivo {device_id}: {anomaly['description']}")
                return anomaly
    ...
    def generate_daily_report(self, date: datetime = None) -> Dict:
        ...
        try:
            ...
            for device in devices:
                ...
                if device_stats:
                    # Detectar anomalias
                    anomaly = self.detect_anomalies(device.id, device_stats["average_power_watts"])
            ...
            for device_report in report["devices"]:
                daily_report = DailyReport(
                    ...
                    anomaly_description=device_report.get("anomaly", {}).get("description")
                )
                db.add(daily_report)
            ...
            logger.info(f"Relatório diário gerado para {date}: {report['total_energy_kwh']:.3f} kWh, R$ {report['total_cost']:.2f}")
            return report
