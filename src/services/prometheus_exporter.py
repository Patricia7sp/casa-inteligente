...
class SmartLifePrometheusExporter:
    ...
    def __init__(self, port=9090):
        ...
        self.smartlife_consumption_daily = Gauge(
            'smartlife_consumption_daily_kwh',
            'Consumo diário da geladeira em kWh'
        )
        ...
        self.smartlife_cost_monthly = Gauge(
            'smartlife_cost_monthly_brl',
            'Custo mensal estimado em R$'
        )
        
        self.smartlife_status = Gauge(
            'smartlife_status',
            'Status do dispositivo (1=normal, 0=alert)'
        )
        
        self.smartlife_last_update = Gauge(
            'smartlife_last_update_timestamp',
        ...
        self.smartlife_info = Info(
            'smartlife_device',
            'Informações do dispositivo SmartLife'
        )
    ...
    def update_metrics(self):
        ...
        try:
            ...
            # Atualizar gauges
            self.smartlife_consumption_daily.set(
            ...
            print(f"  Projeção mensal: {metrics.get('monthly_projection_kwh', 0):.1f} kWh")
            print(f"  Custo mensal: R$ {metrics.get('estimated_monthly_cost_brl', 0):.2f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Prometheus Exporter para SmartLife'
    )
