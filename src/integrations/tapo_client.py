...
class TapoClient:
    ...
    async def add_device(self, ip_address: str, device_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Dispositivo {device_name} ({ip_address}) adicionado com sucesso")
            logger.debug(f"   Modelo: {device_info.model}, FW: {device_info.fw_ver}")
            
            return True
            
        except Exception as e:
            logger.error(f"❌ Erro ao adicionar dispositivo {device_name} ({ip_address}): {str(e)}")
            return False
    ...
    async def get_energy_usage(self, device_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            data = {
                ...
                "current": current_power.current_power / 127000.0 if current_power.current_power > 0 else 0,  # Estimativa
            ...
            logger.debug(f"📊 {device_name}: {data['power_watts']:.1f}W, {data['energy_today_kwh']:.3f}kWh hoje")
