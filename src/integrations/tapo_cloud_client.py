...
class TapoCloudClient:
        ...
        for region in regions:
            try:
                async with aiohttp.ClientSession(connector=ssl_context) as session:
                    ...
                    async with session.get(f"{region}/api/v2/server/info", timeout=5) as response:
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(login_url, json=login_data, timeout=10) as response:
                if response.status == 200:
    ...
    async def refresh_device_list(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(device_list_url, headers=headers, timeout=10) as response:
                if response.status == 200:
                    result = await response.json()
                    
                    if "result" in result and "deviceList" in result["result"]:
                        self.device_list = result["result"]["deviceList"]
                        logger.info(f"Encontrados {len(self.device_list)} dispositivos na cloud")
                        return True
                    else:
    ...
    async def get_device_info(self, device_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self.session.post(device_info_url, headers=headers, json=device_data, timeout=10) as response:
                if response.status == 200:
                    result = await response.json()
                    ...
                    if "result" in result and "responseData" in result["result"]:
                        ...
                        logger.error(f"Erro ao obter info do dispositivo {device_id}: {result}")
                        return None
                else:
                    logger.error(f"Erro HTTP ao obter info do dispositivo: {response.status}")
                    return None
    ...
    async def get_energy_usage(self, device_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if "power_usage" in device_info:
                energy_data.update({
                    "power_watts": device_info["power_usage"].get("current_power", 0),
                    "energy_today_kwh": device_info["power_usage"].get("today_energy", 0),
                    "energy_total_kwh": device_info["power_usage"].get("total_energy", 0)
                })
            
            elif "energy_monitoring" in device_info:
                energy_data.update({
                    "power_watts": device_info["energy_monitoring"].get("current_power", 0),
                    "voltage": device_info["energy_monitoring"].get("voltage", 220.0),
                    "current": device_info["energy_monitoring"].get("current", 0),
                    "energy_today_kwh": device_info["energy_monitoring"].get("today_energy", 0),
                    "energy_total_kwh": device_info["energy_monitoring"].get("total_energy", 0)
                })
    ...
    async def control_device(self, device_id: str, action: str) -> bool:
        ...
        try:
            ...
            async with self.session.post(control_url, headers=headers, json=control_data, timeout=10) as response:
                if response.status == 200:
                    result = await response.json()
                    ...
                    logger.error(f"Erro HTTP ao controlar dispositivo: {response.status}")
                    return False
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            if await self.login():
                ...
                logger.info(f"Conexão TP-Link Cloud OK. {len(devices)} dispositivos encontrados.")
                return True
            return False
