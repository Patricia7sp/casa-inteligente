    ...
    async def _sync_time(self) -> bool:
        ...
        try:
            ...
            for i in range(5):  # Mais medições para melhor precisão
                ...
                async with self.session.get(f"{self.base_url}{path}", headers=headers, timeout=5) as response:
            ...
            if offsets:
                ...
                if len(offsets) > 1:
                    variance = sum((x - median_offset)**2 for x in offsets) / len(offsets)
                    std_dev = variance ** 0.5
                    safety_margin = min(500, std_dev * 2)  # Máximo 500ms de margem
                    self.time_offset += safety_margin if median_offset > 0 else -safety_margin
                
                logger.info(f"Tempo sincronizado. Offset: {median_offset:.0f}ms, ajustado: {self.time_offset:.0f}ms")
                return True
            else:
            ...
            async with self.session.get(f"{self.base_url}{path}", headers=headers) as response:
                if response.status == 200:
                    ...
                    if result.get('success') and 'result' in result:
                        ...
                        self.token_expires = time.time() + result['result']['expire_time'] - 60
            ...
            async with self.session.get(f"{self.base_url}{path}", headers=headers) as response:
                if response.status == 200:
                    ...
                    if result.get('success') and 'result' in result:
                        ...
                        logger.info(f"Encontrados {len(devices)} dispositivos na Tuya Cloud")
                        return devices
                    else:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}{path}", headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Erro HTTP ao obter status do dispositivo: {response.status}")
                    return None
        ...
        try:
            ...
            for item in status_data:
                ...
                if code == 'cur_power':  # Potência atual
                    energy_data['power_watts'] = float(value) / 10  # Geralmente é valor *10
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            if await self._get_token():
                ...
                logger.info(f"Conexão Tuya Cloud OK. {len(devices)} dispositivos encontrados.")
                return True
            return False
