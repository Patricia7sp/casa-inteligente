...
class NovaDigitalClient:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/auth/verify") as response:
                if response.status == 200:
                    ...
                    logger.error(f"Falha na autenticação Nova Digital: {response.status}")
                    return False
    ...
    async def add_device(self, device_id: str, device_name: str) -> bool:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/devices/{device_id}") as response:
                if response.status == 200:
                    ...
                    logger.info(f"Dispositivo Nova Digital {device_name} ({device_id}) adicionado com sucesso")
                    return True
                else:
                    logger.error(f"Dispositivo {device_id} não encontrado: {response.status}")
                    return False
                    
        except Exception as e:
            logger.error(f"Erro ao adicionar dispositivo Nova Digital {device_name}: {str(e)}")
            return False
    ...
    async def get_energy_usage(self, device_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/devices/{device_id}/energy") as response:
                if response.status == 200:
                    ...
                    logger.error(f"Erro ao obter dados de energia do dispositivo {device_name}: {response.status}")
                    return None
    ...
    async def get_historical_data(self, device_name: str, start_date: datetime, end_date: datetime) -> List[Dict]:
        """
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/devices/{device_id}/energy/history", params=params) as response:
                if response.status == 200:
    ...
    async def control_device(self, device_name: str, action: str) -> bool:
        ...
        try:
            ...
            async with self.session.post(f"{self.base_url}/devices/{device_id}/control", json=data) as response:
                if response.status == 200:
                    logger.info(f"Dispositivo {device_name} {action} com sucesso")
                    return True
                else:
                    logger.error(f"Erro ao controlar dispositivo {device_name}: {response.status}")
                    return False
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            if await self.authenticate():
                ...
                logger.info(f"Conexão Nova Digital OK. {len(devices)} dispositivos encontrados.")
                return True
            return False
